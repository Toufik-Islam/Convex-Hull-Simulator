...
public class AnimationArea extends Canvas implements MouseListener
{
    ...
    public void paint(Graphics g) { Graphics2D g2d = (Graphics2D) g;
        g2d.scale(zoomFactor, zoomFactor); // Apply scaling for zoom
    ...
    public void resetZoom() {
        ...
        setPreferredSize(new Dimension(canvasSize, canvasSize)); // Reset size to original
    ...
    public void zoom(double factor) {
        zoomFactor *= factor;
        int newSize = (int) (canvasSize * zoomFactor);
        setPreferredSize(new Dimension(newSize, newSize)); // Update canvas size
        revalidate();
        repaint();
    }
    ...
    public void random(HashSet<Point> points, int numPoints) {
        this.points= points;
        clear();
